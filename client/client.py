...
class Client:
    ...
    def get_resources_info(self):
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            while time() < wait_unitl:
                s.settimeout(wait_unitl - time())

                try:
                    gathered_info.append(
                        self.__get_resources_info_get_response(s, msg_ident))
    ...
    def __get_resources_info_get_response(self, ready_socket, request_ident):
        ...
        data = self.__get_resources_info_process_response_body(
